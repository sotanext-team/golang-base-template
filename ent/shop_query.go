...
type ShopQuery struct {
	...
	withThemeTemplates *ThemeTemplateQuery
...
func (sq *ShopQuery) QueryThemes() *ThemeQuery {
...
// QueryThemeTemplates chains the current query on the "themeTemplates" edge.
func (sq *ShopQuery) QueryThemeTemplates() *ThemeTemplateQuery {
	query := &ThemeTemplateQuery{config: sq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *ShopQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (sq *ShopQuery) FirstIDX(ctx context.Context) uint64 {
...
// Returns a *NotSingularError when exactly one Shop entity is not found.
...
// Returns a *NotSingularError when exactly one Shop ID is not found.
// Returns a *NotFoundError when no entities are found.
func (sq *ShopQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (sq *ShopQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (sq *ShopQuery) IDs(ctx context.Context) ([]uint64, error) {
	var ids []uint64
...
func (sq *ShopQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := sq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (sq *ShopQuery) Clone() *ShopQuery {
	...
	return &ShopQuery{
		...
		withThemes:         sq.withThemes.Clone(),
		withThemeTemplates: sq.withThemeTemplates.Clone(),
...
func (sq *ShopQuery) WithThemes(opts ...func(*ThemeQuery)) *ShopQuery {
	query := &ThemeQuery{config: sq.config}
...
// WithThemeTemplates tells the query-builder to eager-load the nodes that are connected to
// the "themeTemplates" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *ShopQuery) WithThemeTemplates(opts ...func(*ThemeTemplateQuery)) *ShopQuery {
	query := &ThemeTemplateQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withThemeTemplates = query
	return sq
}
...
func (sq *ShopQuery) sqlAll(ctx context.Context) ([]*Shop, error) {
	...
	if query := sq.withThemes; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uint64]*Shop)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Themes = []*Theme{}
		}
		query.Where(predicate.Theme(func(s *sql.Selector) {
			s.Where(sql.InValues(shop.ThemesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.Themes = append(node.Edges.Themes, n)
		}
	}

	if query := sq.withThemeTemplates; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uint64]*Shop)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.ThemeTemplates = []*ThemeTemplate{}
		}
		query.Where(predicate.ThemeTemplate(func(s *sql.Selector) {
			s.Where(sql.InValues(shop.ThemeTemplatesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.ThemeTemplates = append(node.Edges.ThemeTemplates, n)
		}
	}

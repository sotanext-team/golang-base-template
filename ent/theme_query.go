...
func (tq *ThemeQuery) QueryThemeTemplates() *ThemeTemplateQuery {
	query := &ThemeTemplateQuery{config: tq.config}
...
func (tq *ThemeQuery) QueryShop() *ShopQuery {
	query := &ShopQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *ThemeQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (tq *ThemeQuery) FirstIDX(ctx context.Context) uint64 {
...
func (tq *ThemeQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (tq *ThemeQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (tq *ThemeQuery) IDs(ctx context.Context) ([]uint64, error) {
	var ids []uint64
...
func (tq *ThemeQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := tq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (tq *ThemeQuery) Clone() *ThemeQuery {
	...
	return &ThemeQuery{
		...
		withThemeTemplates: tq.withThemeTemplates.Clone(),
		withShop:           tq.withShop.Clone(),
...
func (tq *ThemeQuery) WithThemeTemplates(opts ...func(*ThemeTemplateQuery)) *ThemeQuery {
	query := &ThemeTemplateQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withThemeTemplates = query
	return tq
}
...
func (tq *ThemeQuery) WithShop(opts ...func(*ShopQuery)) *ThemeQuery {
	query := &ShopQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *ThemeQuery) sqlAll(ctx context.Context) ([]*Theme, error) {
	...
	if query := tq.withThemeTemplates; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uint64]*Theme)
		for i := range nodes {
		...
		query.Where(predicate.ThemeTemplate(func(s *sql.Selector) {
			s.Where(sql.InValues(theme.ThemeTemplatesColumn, fks...))
		}))
	...
	if query := tq.withShop; query != nil {

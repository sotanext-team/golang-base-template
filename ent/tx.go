...
func (tx *Tx) init() {
	tx.BkTemplateSection = NewBkTemplateSectionClient(tx.config)
	tx.ComponentProperty = NewComponentPropertyClient(tx.config)
	tx.CustomComponent = NewCustomComponentClient(tx.config)
	tx.GlobalTemplate = NewGlobalTemplateClient(tx.config)
	tx.Shop = NewShopClient(tx.config)
	tx.TemplateSection = NewTemplateSectionClient(tx.config)
	tx.TemplateSectionVersion = NewTemplateSectionVersionClient(tx.config)
	tx.Theme = NewThemeClient(tx.config)
	tx.ThemeTemplate = NewThemeTemplateClient(tx.config)
...
// applies a query, for example: BkTemplateSection.QueryXXX(), the query will be executed
